...
3. **Consultar troubleshooting (CRAG Protocol):**
   - Leer `skills/phva-cycle/troubleshooting/index.md` para buscar errores conocidos.
   - **Aplicar CRAG:** Evaluar la relevancia de la información recuperada según el protocolo Corrective RAG.
   - Si la relevancia es baja (< 5), no improvisar: ejecutar refinamiento de búsqueda o fallback a investigación web/externa.
   - Si hay match de alta relevancia, aplicar la solución documentada de forma preventiva.

4. **Confirmar con el usuario** si hay ambigüedad o si el CRAG sugiere un cambio de estrategia.
